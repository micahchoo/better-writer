  ...
  void draftStoreRef.current
   ...
   .then((annotations) => {
    ...
    const valid = annotations.filter(
     (a) => draftRef.current.slice(a.start, a.end) === a.fragment,
    )
    ...
    <section className="coach-panel" aria-label="Writing coach">
     {sweepNotes.length > 0 ? (
      <p className="coach-question">
       {sweepNotes.length} note{sweepNotes.length === 1 ? '' : 's'} pinned in the text — click a
       highlight to read one.
      </p>
     ) : (
